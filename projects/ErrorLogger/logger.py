...
def get_explanation(error_code, message=None):
    ...
    return message or ERROR_EXPLANATIONS.get(error_code, "Unidentified error (standard)")
...
def get_log_rotation_status():
    ...
    status = {
        ...
        'last_cleanup_check': datetime.fromtimestamp(LAST_CLEANUP) if LAST_CLEANUP else None
    }
    
    if LOG_FILE_PATH and os.path.exists(LOG_FILE_PATH):
        status['current_file_size_mb'] = round(os.path.getsize(LOG_FILE_PATH) / (1024 * 1024), 2)
        status['will_rotate_soon'] = os.path.getsize(LOG_FILE_PATH) >= MAX_FILE_SIZE_BYTES
    else:
    ...
    log_dir = get_log_directory()
    try:
        status['total_log_files'] = len([f for f in os.listdir(log_dir) if f.startswith('errorlog_') and f.endswith('.csv')])
    except OSError:
