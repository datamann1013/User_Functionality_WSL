    ...
    if now - _ollama_cache['last_check'] > 30:  # Cache for 30 seconds
        try:
            ...
            if response.status_code == 200:
                ...
                _ollama_cache['status'] = {'running': False, 'models_available': [], 'last_check': datetime.now().isoformat()}
        except:
            _ollama_cache['status'] = {'running': False, 'models_available': [], 'last_check': datetime.now().isoformat()}
...
def chat():
    ...
    try:
        ...
        response = requests.post(f"{OLLAMA_HOST}/api/generate", json=ollama_payload, timeout=60)
