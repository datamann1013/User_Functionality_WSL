...
class PostgreSQLDatabase:
    ...
    def get_agent(self, agent_id: str) -> Optional[Dict]:
        ...
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    if result:
                        ...
                        if (
                            "temperature" in agent_data
                            and agent_data["temperature"] is not None
                        ):
            ...
            log_error_remote(
                "EADB02",
                f"Failed to get agent {agent_id}",
                exception=e,
    ...
    def create_agent(self, agent_data: Dict) -> Dict:
        ...
        try:
            ...
            log_error_remote(
                "EADB04",
                f'Failed to create agent: {agent_data.get("name", "unknown")}',
                exception=e,
                extra={"service": "ai_service"},
            )
            raise
    ...
    def update_agent(self, agent_id: str, update_data: Dict) -> bool:
        ...
        try:
            ...
            log_error_remote(
                "EADB05",
                f"Failed to update agent {agent_id}",
                exception=e,
                extra={"service": "ai_service", "agent_id": agent_id},
            )
            return False
    ...
    def log_conversation(
        ...
        try:
            ...
            log_error_remote(
                "EADB07",
                f"Failed to log conversation for agent {agent_id}",
                exception=e,
                extra={"service": "ai_service", "agent_id": agent_id},
            )
            return None
        ...
        try:
            with self.get_connection() as conn:
                with conn.cursor() as cur:
                    ...
                    results = cur.fetchall()
                    return [
                        dict(row) for row in reversed(results)
                    ]  # Chronological order

        except Exception as e:
            log_error_remote(
                "EADB08",
                f"Failed to get conversation history for agent {agent_id}",
                exception=e,
                extra={"service": "ai_service", "agent_id": agent_id},
            )
            return []
    ...
    def store_memory(self, agent_id: str, content: str, importance: float = 0.5) -> str:
        ...
        try:
            ...
            log_error_remote(
                "EADB09",
                f"Failed to store memory for agent {agent_id}",
                exception=e,
                extra={"service": "ai_service", "agent_id": agent_id},
            )
            return None
    ...
    def get_agent_memories(
        self, agent_id: str, min_importance: float = 0.0, limit: int = 10
    ) -> List[Dict]:
        ...
        try:
            ...
            log_error_remote(
                "EADB10",
                f"Failed to get memories for agent {agent_id}",
                exception=e,
                extra={"service": "ai_service", "agent_id": agent_id},
            )
            return []
    ...
    def update_memory_access(self, memory_id: str):
        ...
        try:
            ...
            log_error_remote(
                "EADB11",
                f"Failed to update memory access {memory_id}",
                exception=e,
                extra={"service": "ai_service"},
            )
