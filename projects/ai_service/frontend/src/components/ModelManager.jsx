...
const ModelManager = ({ isOpen, onClose }) => {
  ...
  const downloadModel = async (modelName) => {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/models/download/${encodeURIComponent(modelName)}`, {
        method: 'POST',
      });
          ...
          <div className="models-section">
            ...
            <div className="custom-download">
              ...
              <button
                ...
                disabled={!newModelName.trim() || downloadingModels.has(newModelName.trim())}
                className="download-btn"
              >
                Download
              </button>
            ...
            <div className="download-hint">
              Find more models at <a href="https://ollama.ai/library" target="_blank" rel="noopener noreferrer">ollama.ai/library</a>
            </div>
          </div>
        </div>
