...
class BenchmarkAnalyzer:
    ...
    def get_recent_benchmarks(self, benchmarks: List[Dict], days: int = 7) -> List[Dict]:
        ...
        for benchmark in benchmarks:
            try:
                timestamp = datetime.fromisoformat(benchmark['timestamp'].replace('Z', '+00:00'))
    ...
    def analyze_memory_trends(self, memory_benchmarks: List[Dict]) -> Dict[str, any]:
        ...
        for component in components:
            ...
            for benchmark in memory_benchmarks:
                ...
                if comp_data.get('status') == 'success':
                    ...
                    memory_mb = (
                        comp_data.get('peak_memory_mb') or
                        comp_data.get('estimated_runtime_mb') or
                        comp_data.get('memory_after_logs_mb', 0)
                    )
                    if memory_mb > 0:
            ...
            if len(memory_values) >= 2:
                ...
                trends[component] = {
                    ...
                    "regression_detected": trend_analysis['slope'] > self.regression_threshold - 1,
        ...
        overall_trend = self._calculate_trend(total_memory_values) if len(total_memory_values) >= 2 else {}
        
        return {
            "status": "success",
            "component_trends": trends,
            "overall_trend": overall_trend,
            "regression_detected": any(t.get('regression_detected', False) for t in trends.values()),
            ...
            if efficiency_scores or performance_scores:
                ...
                if efficiency_scores:
                    ...
                    trends[component]['efficiency'] = {
                        ...
                        "regression_detected": efficiency_trend['slope'] < -(self.regression_threshold - 1)
                    }
                ...
                if performance_scores:
                    ...
                    trends[component]['performance'] = {
                        ...
                        "regression_detected": performance_trend['slope'] < -(self.regression_threshold - 1)
                    }
    ...
    def detect_regressions(self, analysis_results: Dict[str, any]) -> List[Dict[str, any]]:
                    ...
                    if trend.get('regression_detected', False):
                        regressions.append({
                            ...
                            "severity": self._calculate_regression_severity(
                                trend['current_memory_mb'], trend['average_memory_mb'], "memory"
                            ),
                    ...
                    for metric, trend_data in trends.items():
                        if trend_data.get('regression_detected', False):
                            regressions.append({
                                ...
                                "severity": self._calculate_regression_severity(
                                    trend_data['current_score'], trend_data['average_score'], "cpu"
                                ),
    ...
    def _calculate_regression_severity(self, current: float, average: float, metric_type: str) -> str:
    ...
    def generate_report(self, days: int = 7) -> Dict[str, any]:
        ...
        print(f"   Found {len(recent_memory)} memory benchmarks, {len(recent_cpu)} CPU benchmarks")
...
def main():
    ...
    if summary['total_regressions'] > 0:
        ...
        for regression in report['regressions']:
            print(f"   {regression['severity'].upper()}: {regression['component']} "
